...
public class ComputeMethods {
    ...
    public static int binToDec(String bin) {
        ...
        switch (binLen) {
            case 8:
               dec = 128*numChar(arr, 0) + 64*numChar(arr, 1) + 32*numChar(arr, 2) + 16*numChar(arr, 3) + 8*numChar(arr, 4) + 4*numChar(arr, 5) + 2*numChar(arr, 6) + 1*numChar(arr, 7);
               return dec;
            case 4:
            ...
            default:
                throw new IllegalStateException("Supports only 8bits rn. Inserted binary is " + binLen);
        }
